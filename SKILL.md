---
name: onefetch
description: 读取、总结、翻译任意网页内容，或从页面中提取特定元素。当用户给出一个 URL 并要求查看、阅读、总结、翻译、保存网页内容时使用；当用户要求从网页中提取图片、下载链接、特定字段等元素时也使用；当用户要求总结 B 站视频内容时也使用（通过字幕提取）。支持小红书、微信公众号、知乎、B 站、通用网页及 SPA/JS 渲染页面。
---
...
| B 站 | `bilibili.com/video/*`、`bilibili.com/opus/*` | 视频（AI 字幕提取）、专栏文章 |
...
6. 仅在用户明确要求保存 / 归档时使用 `--store`。默认只保存纯文本，用户要求保存图片时加 `--with-images`。
   - 执行保存前，agent 需主动确认一次：
     - 「是否连图片一起保存？默认仅保存正文，不保存图片。」
   - 若用户补充“也要图片”，再加 `--with-images`。
   - 若内容已保存但后续才决定要图片，可直接使用缓存补下载并重写 `note.md`，不需要重新抓取页面。

### 风险控制抓取节奏（safe 策略）

7. 对“需要登录/Cookie”的平台（或用户明确担心风控）默认建议使用 `safe` 节奏，并先征得用户同意：
   - 建议提示：
     - 「这个站点建议用更稳妥的抓取节奏（会更慢），以降低异常风险。是否按稳妥模式执行？」
   - 若用户同意，agent 采用：
     - 串行执行（一次只处理 1 个 URL，不并发）
     - 分批抓取（每批 5~10 条 URL）
     - URL 之间停顿约 2~6 秒（可抖动）
     - 批次之间停顿约 60~180 秒
     - 默认优先 `--from-cache`，仅用户明确要求“最新”时才 `--refresh`
     - 重试最多 1 次，失败后先告知用户，不连续高频重打
   - 若用户明确要求“正常速度”，可切回 normal（不做额外延时）。
...
7. agent 用 LLM 整理完内容后（生成摘要、要点、标签），**立刻调用 `cache-backfill` 写入缓存**，不要等下次运行：
   ...
   - 生成 `summary` / `key_points` / `tags` 时，必须忽略并删除图片标记：
     - `[IMG:N]`
     - `[IMG_CAPTION:N]`
   - 这些标记只用于图文位置映射，不是正文语义。
   - 保存产物规则：
     - `note.md` 仅在“已有大模型结构化结果”时写入摘要/要点/标签。
     - `feed.json` 固定包含 `llm_outputs`；若暂无大模型结果则为空结构（`summary=""`、`key_points=[]`、`tags=[]`、`extras={}`）。
### 提取图片（images 命令）
...
10. 当用户需要图文一起分析（如多模态模型）时，在 `--present` 上加 `--with-images`：
    ...
    **占位符说明：**
    ...
    - `[IMG_CAPTION:N]` 是图片说明标记，后面跟图片说明文本
    - 默认输出（不带 `--with-images`）的 body 中不含占位符，是干净的纯文本
    - 生成摘要/要点时，不要把 `[IMG:N]`、`[IMG_CAPTION:N]` 当成正文内容
...
6. 当用户要求“批量抓取并保存为合集”时，使用：
   ...
   - `--ingest-store` 时会把 `seed_url` 对应页面也一起保存（不仅是 discovered 列表）。
   - 图片默认不下载；用户明确要求“连图片一起保存”时加 `--ingest-with-images`。

### 扩展联调 smoke（仅维护模式）

- `bash scripts/smoke_extensions.sh` 仅用于开发/发布前联调，不用于普通用户请求。
- 触发时机：扩展仓库刚更新、首次接入新环境、排查“扩展安装成功但抓取异常”。
- 默认用户会话不要主动执行该脚本；日常请求按 `ingest` 主流程处理。

### 面向普通用户的提示模板（建议）

- 当保存文件里暂时没有摘要/要点/标签时：
  - 「正文已保存完成。当前文件还没有大模型整理的摘要/要点/标签；如果你需要，我可以现在基于正文补充整理并回填保存。」
- 当需要刷新网页最新内容时：
