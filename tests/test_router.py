...
def test_route_xiaohongshu_url_to_xhs_adapter() -> None:
    router = Router([XiaohongshuAdapter(), WechatAdapter(), ZhihuAdapter(), XAdapter(), GenericHtmlAdapter()])
...
def test_route_x_status_to_x_adapter() -> None:
    router = Router([XiaohongshuAdapter(), WechatAdapter(), ZhihuAdapter(), XAdapter(), GenericHtmlAdapter()])
    adapter = router.route("https://x.com/someone/status/123")
    assert adapter.id == "x"
