...
def _load_geekbang_adapter_class():
    ...
    path = root / ".onefetch" / "extensions" / "geekbang" / "adapter.py"
    if not path.is_file():
        pytest.skip(f"geekbang extension adapter not installed: {path}")
    spec = importlib.util.spec_from_file_location("onefetch_ext_geekbang_adapter", path)
    if spec is None or spec.loader is None:
        pytest.skip(f"failed to load geekbang extension adapter: {path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    adapter_cls = getattr(module, "GeekbangAdapter", None)
    if adapter_cls is None:
        pytest.skip("GeekbangAdapter class not found in extension module")
    return adapter_cls


def test_geekbang_detail_extracts_author_published_and_image_caption() -> None:
...
def test_geekbang_cleanup_filters_known_noise_lines() -> None:
    adapter_cls = _load_geekbang_adapter_class()
    raw = """
...
def test_geekbang_intro_extracts_article_typo_content() -> None:
    adapter_cls = _load_geekbang_adapter_class()
    tree = html.fromstring(
        """
        <html><body>
