...
How to get it:
  Method 1: F12 DevTools
    1. Open xiaohongshu.com in browser, log in
...
if [[ -n "$clipboard" && "$clipboard" == *"="* && "$clipboard" == *";"* ]]; then
  ...
  echo "[setup_xhs_cookie] detected cookie from clipboard (${#cookie_content} chars)"
  printf '  %.60s...\n' "$cookie_content"
  printf 'Use this? [Y/n] '
  IFS= read -r confirm
...
echo "  bash scripts/setup_xhs_cookie.sh < /path/to/cookie.txt"
echo ""
